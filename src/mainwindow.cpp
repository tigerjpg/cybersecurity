  ...
  ui->login_error_message->setStyleSheet("QLabel { "
    ...
    if(!ui->usernameBox->text().isEmpty() && !ui->passwordBox->text().isEmpty())
    {
      if(db->Authenticate(ui->usernameBox->text(), ui->passwordBox->text()))
      {
        ...
        ui->customer_view->setModel(cTableModel);
        ui->customer_view->hideColumn(CustomerTableModel::ID);
        ui->customer_view->hideColumn(CustomerTableModel::INTEREST);
        ui->customer_view->hideColumn(CustomerTableModel::KEY);
